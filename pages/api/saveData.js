...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        const { username, email, password, userId, name } = req.body; // Extract username from the request body
        ...
        try {
            ...
            if (isAvailable) {
                await collection.insertOne({ username, email, hashedPassword, userId, name });
